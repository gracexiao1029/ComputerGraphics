...
float improved_perlin_noise( vec3 st) 
{
	...
	float mixed_x1 = mix(dot(d1, f1), dot(d2, f2), improved_smooth_step(frac).x);
	float mixed_x2 = mix(dot(d3, f3), dot(d4, f4), improved_smooth_step(frac).x);
	float mixed_x3 = mix(dot(d5, f5), dot(d6, f6), improved_smooth_step(frac).x);
	float mixed_x4 = mix(dot(d7, f7), dot(d8, f8), improved_smooth_step(frac).x);
	float mixed_y1 = mix(mixed_x1, mixed_x2, improved_smooth_step(frac).y);
	float mixed_y2 = mix(mixed_x3, mixed_x4, improved_smooth_step(frac).y);
	float mixed_z = mix(mixed_y1, mixed_y2, improved_smooth_step(frac).z);
