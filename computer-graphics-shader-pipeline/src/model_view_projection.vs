...
void main()
{
    if (is_moon) {
      ...
      vec4 transformation = model(is_moon, animation_seconds) * vec4(pos_vs_in, 1.0) + shift;
      pos_cs_in = proj * view * transformation;
    } else {
      pos_cs_in =  proj * view * identity() * vec4(pos_vs_in, 1.0); 
    }
}
